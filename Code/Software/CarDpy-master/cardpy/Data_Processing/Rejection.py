def shot_rejection(original_matrix, original_bvals, original_bvecs, NRMSE_threshold = 0.75, NSSIM_threshold = 0.75, zoom = 'ON', IntERACT_zoom = 'ON', organ = 'Heart', operation_type = 'Magnitude'):
    ...
    for slc in range(slices):                                                                                                                                       # Iterate through slices
        for avg_i in range(len(bval_low_indicies)):                                                                                                                                   # Iterate through averages (ith average)
            for avg_j in range(len(bval_low_indicies)):                                                                                                                                   # Iterate through averages (jth average)
                SSIM_bvl_v_bvl[avg_i, avg_j, slc] = ssim(abs(original_matrix_stacked[y_start[slc]:y_end[slc], x_start[slc]:x_end[slc], slc, bval_low_indicies[avg_i]]),
                                                         abs(original_matrix_stacked[y_start[slc]:y_end[slc], x_start[slc]:x_end[slc], slc, bval_low_indicies[avg_j]]))                           # Compute SSIM between ith and jth low b-value averages
                RMSE_bvl_v_bvl[avg_i, avg_j, slc] = np.sqrt(mse(abs(original_matrix_stacked[y_start[slc]:y_end[slc], x_start[slc]:x_end[slc], slc, bval_low_indicies[avg_i]]),
                                                                abs(original_matrix_stacked[y_start[slc]:y_end[slc], x_start[slc]:x_end[slc], slc, bval_low_indicies[avg_j]])))                   # Compute RMSE between ith and jth low b-value averages
    ...
    for slc in range(slices):                                                                                                                                       # Iterate through slices
        for avg_i in range(len(bval_high_indicies)):                                                                                                                                   # Iterate through averages (ith average)
            for avg_j in range(len(bval_high_indicies)):                                                                                                                                   # Iterate through averages (jth average)
                SSIM_bvh_v_bvh[avg_i, avg_j, slc] = ssim(abs(original_matrix_stacked[y_start[slc]:y_end[slc], x_start[slc]:x_end[slc], slc, bval_high_indicies[avg_i]]),
                                                         abs(original_matrix_stacked[y_start[slc]:y_end[slc], x_start[slc]:x_end[slc], slc, bval_high_indicies[avg_j]]))                           # Compute SSIM between ith and jth low b-value averages
                RMSE_bvh_v_bvh[avg_i, avg_j, slc] = np.sqrt(mse(abs(original_matrix_stacked[y_start[slc]:y_end[slc], x_start[slc]:x_end[slc], slc, bval_high_indicies[avg_i]]),
                                                                abs(original_matrix_stacked[y_start[slc]:y_end[slc], x_start[slc]:x_end[slc], slc, bval_high_indicies[avg_j]])))                   # Compute RMSE between ith and jth low b-value averages
    ...
    accepted_matrix = np.zeros(original_matrix.shape,dtype = original_matrix_stacked.dtype)
    accepted_bvals  = []
    accepted_bvecs  = []
...
#                        accepted_matrix[:, :, slc, dif, 0] = temp_image
        ...
        accepted_bvals  = original_bvals
        accepted_bvecs  = original_bvecs
        print('DATA TYPE',accepted_matrix.dtype)
